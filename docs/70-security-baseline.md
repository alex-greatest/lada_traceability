...
Принятое policy-исключение (accepted residual risk):
- Scope: только `App.config` и только ключи `connectMySql_prod` и `connectMySql_test`.
- Статус: разрешено хранение plaintext credentials.
- Срок действия: бессрочно.
- Дополнительные compensating controls: отсутствуют.
- Rationale: операционная совместимость с текущей моделью поставки/запуска в производственном контуре.
- Source-of-truth: `AGENTS.md` (раздел `Hard Safety Rules`, пп.5-6).
...
2. Ограничить доступ к `App.config` и артефактам с секретами (рекомендация; не является обязательным control для принятого исключения `connectMySql_prod/connectMySql_test`).
...
2. Нет новых секретов в коде/конфигах, кроме принятого исключения: `App.config` ключи `connectMySql_prod/connectMySql_test`.
